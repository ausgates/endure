  ...
  def eat(food)
    ...
    if @foods.include? food
      ...
      if food == 'pills'
        ...
        # eat all food
#################################
        hunger += @foods.length * 10 #Adds 10 hunger points for each food item avialable
        @foods.clear #Gets rid off all the food, because you just ate it
#################################
      else
        @hunger += 10
      end
      ...
      puts Rainbow('You are too full to eat right now').red #########
      pause #########################################################
    else
  ...
  def trash(thing)
    if @foods.include? thing
      puts Rainbow("Are you sure you want to trash your #{thing}? [y/n ]").red #This just doesn't sound right
  ...
  def add_friend(friend)
    @friends.push(friend)
    puts "#{friend} was added to your friend's list."
  end
